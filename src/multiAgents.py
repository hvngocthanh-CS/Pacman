    ...
    def getAction(self, gameState):
        ...
        def min_value(state, agentIndex, depth, alpha, beta):
            ...
            for action in legalActions:
                ...
                if agentIndex == (state.getNumAgents() - 1):
                    new_value = max_value(newState, depth, alpha, beta)
                else:
                    new_value = min_value(newState, agentIndex + 1, depth, alpha, beta)

                val = min(val, new_value)
                if val < alpha:
                    return val
                beta = min(beta, val)
            return val
        ...
        def max_value(state, depth, alpha, beta):
            ...
            for action in legalActions:
                ...
                new_value = min_value(newState, 0 + 1, depth + 1, alpha, beta)
                if new_value > val:
                    val = new_value
                    if depth == 0:
                        bestAction = action
        ...
        bestAction = max_value(gameState, 0, -infinity, infinity)
        return bestAction
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def max_value(state, depth=0):
            ...
            val = -infinity
            legalActions = state.getLegalActions(0)
            succState = [state.generateSuccessor(0, act) for act in legalActions]
            for each in succState:
                val = max(val, exp_value(each, depth, state.getNumAgents()-1))
            return val


        def exp_value(state, depth=0, agentIndex=0):
            if state.isLose() or state.isWin() or depth == 0:
                return self.evaluationFunction(state)
            
            legalActions = state.getLegalActions(agentIndex)
            succState = [state.generateSuccessor(agentIndex, act) for act in legalActions]
            totalexpectedvalue = 0
            for each in succState:
                if agentIndex > 1:
                    totalexpectedvalue += exp_value(each, depth, agentIndex-1)
                else:
                    totalexpectedvalue += max_value(each, depth-1)
            return float(totalexpectedvalue)/len(succState)
        ...
        for action in legalActions:
            ...
            score = exp_value(nextState, self.depth, gameState.getNumAgents()-1)
...
def new_betterEvaluationFunction(currentGameState):
    """
    Your extreme ghost-hunting, pellet-nabbing, food-gobbling, unstoppable
    evaluation function (question 5).
    ...
    numberOfCapsulesLeft = len(newCapsules)
    ...
    distanceToClosestScaredGhost = -1
    ...
    score = 1 * currentScore - 1.5 * distanceToClosestFood - 2 * (1.0/distanceToClosestActiveGhost) - 2 * distanceToClosestScaredGhost - 20 * numberOfCapsulesLeft - 4  * numberOfFoodsLeft
    return score

better = new_betterEvaluationFunction
